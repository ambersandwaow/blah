SMODS.Booster {
    ...
    create_card = function(self, card, i)
        return {set = 'blah_quest', area = G.pack_cards, skip_materialize = true, soulable = true, key_append = 'questPackGen', }
    end,
...
SMODS.Booster {
    ...
    kind = 'questPack',
    create_card = function(self, card, i)
        return create_card("blah_quest", G.blah_quests, nil, nil, true, true, nil, "medQuestPack")
    end,
    loc_vars = function(self, info_queue, card)
        return { vars = { math.min(card.ability.choose + (G.GAME.modifiers.booster_choice_mod or 0), math.max(1, card.ability.extra +
        (G.GAME.modifiers.booster_size_mod or 0))), math.max(1, card.ability.extra + (G.GAME.modifiers.booster_size_mod or 0)) } }
